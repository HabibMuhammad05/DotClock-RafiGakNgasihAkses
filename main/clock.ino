...
#define DIN_PIN 13                        // Data Pin - Nodemcu D8
#define CS_PIN  15                        // Chip Select - Nodemcu D7
#define CLK_PIN 14                        // Clock In - Nodemcu D6

//set false to not display the text before date scrolling.
bool enableTeks = true;
bool enableDHT = true;
const char* teks = " Matrix NTP Clock - ";
...
void displayClock(){
  ...
  if(millis()-clkTime > 30000 && !del && dots) {
    ...
    delay(5);
    if(enableTeks) printStringWithShift(teks,40);
    delay(5);
